...
float getShadow() {
    ...
    float count = 1.0;
    shadow += (shadowCoord.z - bias > texture2D(shadow, shadowCoord.xy + vec2(texelSize.x, 0.0)).r) ? 0.5 : 1.0;
    shadow += (shadowCoord.z - bias > texture2D(shadow, shadowCoord.xy + vec2(-texelSize.x, 0.0)).r) ? 0.5 : 1.0;
    shadow += (shadowCoord.z - bias > texture2D(shadow, shadowCoord.xy + vec2(0.0, texelSize.y)).r) ? 0.5 : 1.0;
    shadow += (shadowCoord.z - bias > texture2D(shadow, shadowCoord.xy + vec2(0.0, -texelSize.y)).r) ? 0.5 : 1.0;
    count += 4.0;
